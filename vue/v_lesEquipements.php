...
<div class="col-sm-6">
	<section class="panel">
		...
		<div class="panel-body">
			...
			<?php
			foreach ($tbEquipements as $equipement) {
			?>
			  <tr>
			  
				<!-- formulaire pour modifier et supprimer les equipements-->
				<form action="index.php?uc=gererEquipements" method="post">
				<td><?php echo $equipement->identifiant; ?><input type="hidden"  name="txtIdEquipement" value="<?php echo $equipement->identifiant; ?>" /></td>
				<td><?php 
					if ($equipement->identifiant != $idEquipementModif) {
						echo $equipement->libelle;
						?>
						</td><td>
							<?php if ($notification != 'rien' && $equipement->identifiant == $idEquipementNotif) {
					...
					<?php
						...
						?><input type="text" id="txtLibEquipement" name="txtLibEquipement" size="32" required minlength="4"  maxlength="40"   value="<?php echo $equipement->libelle; ?>" />
						</td>
						<td>		 
